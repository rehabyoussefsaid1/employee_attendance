...
class _EditProfilePageState extends State<EditProfilePage> {
  ...
  void getUser() async {
    ...
    if (response.error == null) {
      setState(() {
        ...
        loading = false;
        _firstName.text = user!.data.fullName ?? '';
        _lastName.text = user!.data.fullName ?? '';
        _email.text = user!.data.email ?? '';
        _phoneNumber.text = user!.data.phoneNumber ?? '';
        _dateController.text = user!.data.lastLoginDate ?? '';
        _adress.text = user!.data.fullName ?? '';
  ...
  void updateProfile() async {
    ApiResponse response = await updateUser(
        ...
        _adress.text,
        (getStringImage(_image) ?? await getBase64Image(''))
            as String);
